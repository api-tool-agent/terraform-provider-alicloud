...
func resourceAliCloudApigGatewayDelete(d *schema.ResourceData, meta interface{}) error {
	...
	if enableDelete {
		...
		apigServiceV2 := ApigServiceV2{client}
		stateConf := BuildStateConf([]string{}, []string{"Deleted"}, d.Timeout(schema.TimeoutDelete), 30*time.Second, apigServiceV2.ApigGatewayStateRefreshFunc(d.Id(), "status", []string{}))
		if _, err := stateConf.WaitForState(); err != nil {
			return WrapErrorf(err, IdMsg, d.Id())
		}
