	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			},
			{
				...
				// user_data is returned by DescribeUserData as base64, which cannot be
				// compared against the plaintext config value right after import.
				ImportStateVerifyIgnore: []string{"security_enhancement_strategy", "user_data", "dry_run"},
			},
			{
