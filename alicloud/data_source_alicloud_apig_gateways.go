...
func dataSourceAliCloudApigGatewayRead(d *schema.ResourceData, meta interface{}) error {
	...
	query["gatewayId"] = StringPointer(d.Get("gateway_id").(string))
	...
	for {
		...
		for _, v := range result {
			...
			if len(idsMap) > 0 {
				if _, ok := idsMap[fmt.Sprint(item["$.data.gatewayId"])]; !ok {
					continue
				}
			}
	...
	for _, objectRaw := range objects {
		...
		mapping["id"] = objectRaw["$.data.gatewayId"]
		...
		mapping["payment_type"] = objectRaw["chargeType"]
		...
		id := fmt.Sprint(objectRaw["$.data.gatewayId"])
