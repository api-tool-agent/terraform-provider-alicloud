...
* `tag` - (Optional) A tag of the instance. See [`tag`](#tag) below.
...
-> **NOTE:** `resource_group_id` and `tag` are not returned by the KVCacheStore read API, so they will be empty in the state after `terraform import`.
