...
* `resource_group_id` - (Optional) The ID of the resource group used to filter the results.
...
* `stores` - A list of Kv Cache Store entries. Each element contains the following attributes:
    ...
    * `tag` - The tags of the instance. **NOTE:** This field is only available when `enable_details` is `true`.
