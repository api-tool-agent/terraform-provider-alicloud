...
lines. Filenames normally match registration names; the two existing
`ess_lifecyclehooks` and `yundun_dbaudit_instances` exceptions are mapped
explicitly. An existing file with an unresolved registration fails; only a
missing file whose former registration was removed is skipped. Git renames are
expanded into deletion and addition paths, so moving a file does not remove
coverage while its resource remains registered.
